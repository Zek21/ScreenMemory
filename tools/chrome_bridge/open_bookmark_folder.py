...
def build_args(profile_directory: str, folder_id: str, urls: List[str], mode: str) -> List[str]:
    args = [f'--profile-directory={profile_directory}']  # no inner quotes — subprocess list handles quoting (INCIDENT_019 BUG 2)
...
def _open_profile_only(chrome_path: str, profile: dict) -> int:
    ...
    subprocess.Popen(
        [chrome_path, f"--profile-directory={profile['directory']}"],  # no inner quotes (INCIDENT_019 BUG 2)
